...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.82.01), October 15, 2015, at 10:30
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='T:\\admin\\Technical\\training\\PsychoPy-Part2\\Staff-only-files\\ex5-image-ratings-complete\\experiment.psyexp',
...
rating_text = visual.TextStim(win=win, ori=0, name='rating_text',
    text='1 = Not very much,  5 = Very much!',    font='Arial',
    pos=[0, 0], height=0.1, wrapWidth=None,
    color='white', colorSpace='rgb', opacity=1,
    depth=0.0)
question_text_display = visual.TextStim(win=win, ori=0, name='question_text_display',
...
text = visual.TextStim(win=win, ori=0, name='text',
    text='Thank you for participating in this experiment!',    font='Arial',
    pos=[0, 0], height=0.1, wrapWidth=None,
    color='white', colorSpace='rgb', opacity=1,
    depth=0.0)
...
image_loop = data.TrialHandler(nReps=2, method='random', 
    extraInfo=expInfo, originPath='T:\\admin\\Technical\\training\\PsychoPy-Part2\\Staff-only-files\\ex5-image-ratings-complete\\experiment.psyexp',
...
for thisImage_loop in image_loop:
    ...
    frameN = -1
    # update component parameters for each repeat
    break_response = event.BuilderKeyResponse()  # create an object of type KeyResponse
    break_response.status = NOT_STARTED
    #Increase the trial counter for each routine
    myCount = myCount + 1
    # keep track of which components have finished
    break_2Components = []
    break_2Components.append(break_response)
    break_2Components.append(break_text)
    ...
    image_displayComponents.append(image)
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if image.status == STARTED and t >= (0.0 + (3-win.monitorFramePeriod*0.75)): #most of one frame period left
    ...
    for thisQuestion_loop in question_loop:
        ...
        # keep track of which components have finished
        question_displayComponents = []
        question_displayComponents.append(rating_text)
        question_displayComponents.append(question_text_display)
        question_displayComponents.append(question_answer)
        ...
        while continueRoutine:
            ...
            if t >= 0.0 and rating_text.status == NOT_STARTED:
                # keep track of start time/frame for later
                rating_text.tStart = t  # underestimates by a little under one frame
            ...
            if t >= 0.0 and question_text_display.status == NOT_STARTED:
                # keep track of start time/frame for later
                question_text_display.tStart = t  # underestimates by a little under one frame
        ...
        # store data for question_loop (TrialHandler)
